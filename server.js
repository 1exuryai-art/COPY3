...
async function readSiteContent() {
  ...
  try {
    ...
    return { ...EMPTY_SITE_CONTENT, ...JSON.parse(raw) };
  } catch (error) {
...
app.get("/api/admin/content", async (_req, res) => {
  try {
    ...
    return res.status(500).json({ ok: false, error: "Failed to read content" });
  }
});
...
app.post("/api/admin/content", requireAdminAuth, async (req, res) => {
  try {
    ...
    await ensureContentDir();
    await writeFile(CONTENT_FILE, `${JSON.stringify({ ...EMPTY_SITE_CONTENT, ...body }, null, 2)}\n`, "utf8");
